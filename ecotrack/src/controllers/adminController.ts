  ...
  try {
    ...
    const kpiTargets: KPITarget[] = [
      ...
      },
      {
        ...
        target: 10000,
        current: await prisma.order.aggregate({
          where: {
            createdAt: {
              gte: currentMonth
            },
...
export const getTariffSettings = async (req: Request, res: Response) => {
  try {
    const tariffSettings: TariffSettings[] = [
      {
        id: '1',
...
export const getAllUsers = async (req: Request, res: Response) => {
  try {
    ...
    const usersWithStatus = users.map(user => ({
      ...user,
      active: true // Можно рандомизировать или получать из БД
    }));
