...
export const analyticsService = {
  async generateComprehensiveReport(filters: any, auth: any) {
  ...
  async getAdvancedAnalytics(auth: any) {
    ...
    try {
      ...
      for (let i = 11; i >= 0; i--) {
        ...
        const monthName = format(monthStart, 'MMM', { locale: ru });
        ...
        const materialStats = monthOrders.reduce((acc, order) => {
          ...
          if (!acc[material]) acc[material] = { volume: 0, expenses: 0 };
      ...
      return {
        ...
        procurementData: monthlyData,        materialAnalysis: Object.entries(materialAnalysis).map(([material, data]: [string, MaterialData]) => ({
          name: material,
  ...
  generateForecast(recentData: any[]) {
  ...
  async getDemandForecast(months: number) {
  ...
  async createKPIAlert(alertData: any) {
  ...
  async getKPIHistory(kpiId: string, period: string) {
