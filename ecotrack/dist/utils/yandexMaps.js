...
async function geocodeAddress(address) {
    ...
    try {
        const response = await axios_1.default.get(YANDEX_GEOCODE_API_URL, {
            params: {
...
async function getRegionFromAddress(address) {
    try {
        ...
        const region = addressDetails.find((comp) => comp.kind === 'province')?.name;
        if (region) {
            return region;
        }
        // Если не нашли регион, ищем locality
        const locality = addressDetails.find((comp) => comp.kind === 'locality')?.name;
        if (locality) {
